...
void heart(){
  ...
  if(Sharp.left||Sharp.middle||Sharp.right||Bumper.left||Bumper.right){
...
void capital(){
  ...
  if((Sharp.left||Sharp.middle||Sharp.right||Bumper.left||Bumper.right)&&confiance>0){
    neo_leds[random_led] = CRGB(255, 255, 255);
  }
  // brightness
  FastLED.setBrightness(omega);
  FastLED.show();
}
...
void init_neo_pixel() {
  ...
  timerNeoLeds.begin(capital, 10000);
